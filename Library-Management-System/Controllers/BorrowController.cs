...
public class BorrowController(IBookRepository bookRepository,IBorrowRepository borrowRepository): Controller
{
    ...
    public async Task<IActionResult> Create(BorrowViewModel model)
    {
        ...
        try
        {
            var result = await borrowRepository.Create(model);
            if (result == null)
            {
            ...
            {
                TempData["ErrorMessage"] = $"The book '{result.Title}' is already borrowed.";
                return View("NotAvailable");
            }
            TempData["SuccessMessage"] = $"Successfully borrowed the book: {result.Title}.";
    ...
    public async Task<IActionResult> Return(int? borrowRecordId)
    {
        ...
        try
        {
            var borrowRecord = await borrowRepository.GetBorrowRecord(borrowRecordId);
    ...
    public async Task<IActionResult> Return(ReturnViewModel model)
    {
        ...
        try
        {
            var borrowRecord = await borrowRepository.Return(model);
            if (borrowRecord == null)
            {
